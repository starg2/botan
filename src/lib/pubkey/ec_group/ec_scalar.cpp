...
EC_Scalar::EC_Scalar(std::shared_ptr<EC_Group_Data> group, std::unique_ptr<EC_Scalar_Data> scalar) :
      m_group(std::move(group)), m_scalar(std::move(scalar)) {}

EC_Scalar::EC_Scalar(const EC_Group& group, std::unique_ptr<EC_Scalar_Data> scalar) :
      m_group(group._data()), m_scalar(std::move(scalar)) {}

EC_Scalar::EC_Scalar(const EC_Scalar& other) : m_group(other.m_group), m_scalar(other.inner().clone()) {}

EC_Scalar::EC_Scalar(EC_Scalar&& other) noexcept :
      m_group(std::move(other.m_group)), m_scalar(std::move(other.m_scalar)) {}
...
EC_Scalar EC_Scalar::from_bigint(const EC_Group& group, const BigInt& bn) {
   BOTAN_ARG_CHECK(bn > 0 && bn <= group._data()->order(), "EC_Scalar::from_bigint out of range");
...
EC_Scalar EC_Scalar::gk_x_mod_order(const EC_Scalar& scalar, RandomNumberGenerator& rng, std::vector<BigInt>& ws) {
   auto group = scalar.group();
   return EC_Scalar(group, group->gk_x_mod_order(scalar.inner(), rng, ws));
}

std::vector<uint8_t> EC_Scalar::serialize() const {
   return group()->scalar_serialize(inner());
}

std::vector<uint8_t> EC_Scalar::serialize_pair(const EC_Scalar& r, const EC_Scalar& s) {
   BOTAN_ARG_CHECK(r.group() == s.group(), "Curve mismatch");
   return r.group()->scalar_serialize_pair(r.inner(), s.inner());
}
...
std::optional<std::pair<EC_Scalar, EC_Scalar>> EC_Scalar::deserialize_pair(const EC_Group& group,
   ...
   auto r = EC_Scalar::deserialize(group, bytes.subspan(0, half));
   auto s = EC_Scalar::deserialize(group, bytes.subspan(half, half));
