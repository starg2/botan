...
EC_Scalar gost_msg_to_scalar(const EC_Group& group, std::span<const uint8_t> msg) {
   std::vector<uint8_t> rev_bytes(msg.begin(), msg.end());

   for(size_t i = 0; i != rev_bytes.size() / 2; ++i) {
      std::swap(rev_bytes[i], rev_bytes[rev_bytes.size() - 1 - i]);
   }
...
secure_vector<uint8_t> GOST_3410_Signature_Operation::raw_sign(const uint8_t msg[],
   ...
   const auto rs = EC_Scalar::serialize_pair(s, r);
...
bool GOST_3410_Verification_Operation::verify(const uint8_t msg[],
   ...
   if(auto rs = EC_Scalar::deserialize_pair(m_group, std::span{sig, sig_len})) {
      ...
      if(r.is_nonzero() && s.is_nonzero()) {
         ...
         const auto z1 = s * v;
         const auto z2 = r.negate() * v;

         if(const auto w = m_gy_mul.mul2_x_mod_order(z1, z2)) {
            return (w == r);
         }
      }
   }
